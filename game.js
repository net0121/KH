(() => {
  ...
  let activeBarriers = [];
  let activeProjectiles = []; // Added to track enemy bullets
  ...
  function getSpellName(index) {
    ...
    const suffixes = [
      ...
      ['Reflect', 'Reflera', 'Reflega'] // Added Reflect names
    ];
  ...
  // Bind keys 1-4 for quick casting
  ...
  function updateBarriers() {
    for (let i = activeBarriers.length - 1; i >= 0; i--) {
      ...
      // Note: Reflega barrier stays in place and checks for projectile collisions in stepMovement.
    }
  }
  ...
  function spawnSlot(idx) {
    ...
    // Temporarily append to read actual size or use root var approx
    ...
    const slotObj = {
      ...
      attackTimer: 3 + Math.random() * 4 // Add attack cooldown
    };
  ...
  function stepMovement(dt) {
    ...
    // 2. Projectiles & Reflect Logic[cite: 1]
